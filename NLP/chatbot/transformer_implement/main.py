...
def train(encoder, decoder, batch_data, encoder_optim, decoder_optim, mode='parralel'):
    ...
    if mode == 'parralel':
        ...
        loss, valid = MaskedNLL(pred, decoder_target.unsqueeze(-1), mask.unsqueeze(-1))
        total_loss = loss
        valid_pos = valid

    else:
        decoder_input = torch.tensor([[1] for i in range(batch_size)]).to(device) #SOS tag
        target = torch.cat([target, torch.tensor([[2] for i in range(batch_size)], dtype=torch.long).to(device)], dim=0)
...
if __name__ == "__main__":
    ...
    print(train(encoder, decoder, sample, encoder_optimizer, decoder_optimizer))